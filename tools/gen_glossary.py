...
TERMS = [
    ...
    ("ComputeInstanceTransformsAtTime", "Point Instance 1体ごとの変換を行列の配列で返すPython APIです。<code>positions</code>・<code>orientations</code>・<code>scales</code>と、既定ではPrototype root自身の変換も含みます。返る行列はPointInstancerのローカル空間なので、world空間が要るときは<code>ComputeLocalToWorldTransform()</code>を掛けます。"),
